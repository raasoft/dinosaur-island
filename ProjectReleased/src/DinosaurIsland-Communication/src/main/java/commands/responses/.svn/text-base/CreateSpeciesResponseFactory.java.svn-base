 ...
 *
 */
public class CreateSpeciesResponseFactory extends ResponseFactory {
	
	static final CreateSpeciesResponseFactory uniqueInstance = new CreateSpeciesResponseFactory();

	public static CreateSpeciesResponseFactory getInstance() {
		return uniqueInstance;
	}
	
	private CreateSpeciesResponseFactory() {
		super();
	}
	
	public CreateSpeciesResponse createResponse(String theStringToBeParsed) throws IllegalArgumentException {
		...
		boolean speciesAlreadyCreated = false;
		boolean invalidToken = false;
		
		if (args.size() > 0 && args.get(0).isEmpty() == false) {
			if (args.get(0).equals("@razzaGiaCreata")) {
				speciesAlreadyCreated = true;
			} else if (args.get(0).equals("@nomeRazzaOccupato")) {
		...
		return new CreateSpeciesResponse(getResponsePositivity(), nameAlreadyTaken, speciesAlreadyCreated, invalidToken);
	}
	

	@Override
	public void setupLogger() {
		logger = Logger.getLogger("client.responseFactory.createSpeciesResponseFactory");
	...
	protected void populateRegexpNegativeArguments() {
		regexpPatternNegativeArguments.add(new String("(@razzaGiaCreata|@nomeRazzaOccupato|@tokenNonValido)?"));
	}
}

 ...
 *
 */
public class LocalViewResponse extends Response {
	...
	/**
	 * @param localView the localView to set
	 */
	public void setLocalView(LocalView theLocalView) {
		localView = theLocalView;
	}
	public LocalViewResponse(boolean theOutcome, LocalView theLocalView, boolean theInvalidTokenFlag, boolean theInvalidDinosaurIDFlag, boolean thePlayerNotInGameFlag) {
		setOutcomePositive(theOutcome);
		setLocalView(theLocalView);
	...
	public String generateVerbose() {
		if (isOutcomePositive() == true) {
			return new String("@vistaLocale,"+localView.toString());
		}
		else {

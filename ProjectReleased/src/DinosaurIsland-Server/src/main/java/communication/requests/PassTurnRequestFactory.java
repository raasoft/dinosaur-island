 ...
 *
 */
public class PassTurnRequestFactory extends RequestFactory {
	
static final PassTurnRequestFactory uniqueInstance = new PassTurnRequestFactory();
	
	private PassTurnRequestFactory() {
		super();
	}
	
	public PassTurnRequest createRequest(ArrayList<String> theTokens, SocketClientHandler socketClientHandler) throws IllegalArgumentException {
		...
		PassTurnRequest command = new PassTurnRequest(token, socketClientHandler);
		return command;
	}
	
	public static PassTurnRequestFactory getInstance() {
	...
	public void setupLogger() {
		logger = Logger.getLogger("server.commandFactory.requestfactory.passTurnRequestFactory");
	...
	protected void populateValidCommandTags() {
		validCommandTags.add("@passaTurno");		
	}

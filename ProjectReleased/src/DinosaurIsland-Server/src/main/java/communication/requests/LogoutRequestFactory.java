 ...
 *
 */
public class LogoutRequestFactory extends RequestFactory {
	
static final LogoutRequestFactory uniqueInstance = new LogoutRequestFactory();
	
	private LogoutRequestFactory() {
		super();
	}
	
	public LogoutRequest createRequest(ArrayList<String> theTokens, SocketClientHandler socketClientHandler) throws IllegalArgumentException {
		...
		LogoutRequest command = new LogoutRequest(token, socketClientHandler);
		return command;
	}
	
	public static LogoutRequestFactory getInstance() {
	...
	public void setupLogger() {
		logger = Logger.getLogger("server.commandFactory.requestfactory.logoutRequestFactory");

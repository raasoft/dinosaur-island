 ...
 *
 */
public class LocalViewRequestFactory extends RequestFactory {
	
static final LocalViewRequestFactory uniqueInstance = new LocalViewRequestFactory();
	
	private LocalViewRequestFactory() {
		super();
	}
	
	public LocalViewRequest createRequest(ArrayList<String> theTokens, SocketClientHandler socketClientHandler) throws IllegalArgumentException {
		...
		LocalViewRequest command = new LocalViewRequest(token, dinosaurID, socketClientHandler);
		return command;
	}
	
	public static LocalViewRequestFactory getInstance() {
	...
	public void setupLogger() {
		logger = Logger.getLogger("server.commandFactory.requestfactory.localViewRequestFactory");
	...
	protected void populateValidCommandTags() {
		validCommandTags.add("@vistaLocale");		
	}
